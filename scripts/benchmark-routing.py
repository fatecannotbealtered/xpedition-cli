...
function = next(node for node in original_tree.body if isinstance(node, ast.FunctionDef) and node.name == "check_plan")
...
for case in range(200):
    board = current.Board({})
    board.segments = [((rng.uniform(0, 10), rng.uniform(0, 10)),
                       (rng.uniform(0, 10), rng.uniform(0, 10)), rng.choice((1, 2)),
                       rng.choice(("A", "B", "GND")), 0.254, -1) for _ in range(8)]
    board.vias = [((rng.uniform(0, 10), rng.uniform(0, 10)), rng.choice(("A", "B")), -1) for _ in range(3)]
    board.pads = [((1, 1, 2, 2), "A", 1, "U1"), ((4, 4, 5, 5), "B", 2, "U2")]
    items = [{"net": rng.choice(("A", "B", "GND")), "layer": rng.choice((1, 2)),
              "points": [(rng.uniform(0, 10), rng.uniform(0, 10)),
                         (rng.uniform(0, 10), rng.uniform(0, 10))]} for _ in range(4)]
    items += [{"kind": "via", "net": rng.choice(("A", "B")), "at": (rng.uniform(0, 10), rng.uniform(0, 10))} for _ in range(2)]
    assert current.check_plan(items, board) == original(items, board), f"Differential case {case} changed"
...
for count in (1000, 3000, 10000):
    ...
    rows.append({"existing_segments": count, "new_segments": 1, "baseline_median_ms": before,
                 "patched_median_ms": after, "speedup": before / after, "samples_ms": samples})
print(json.dumps({"baseline_commit": BASE, "python": sys.version, "platform": platform.platform(),
                  "fixture": "same-net, no pads/vias; loop overhead only", "differential_cases": 200,
