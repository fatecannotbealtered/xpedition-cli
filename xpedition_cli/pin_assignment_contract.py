...
def commands() -> list[dict[str, Any]]:
    ...
    for verb, description in (
        (
            "pin-plan",
            (
                "Plan explicit pin/net assignments against a suppli"
                "ed snapshot, without writing a design"
            ),
        ),
        (
            "pin-check",
            (
                "Compare requested pin/net assignments with supplie"
                "d observations, not live native verification"
            ),
        ),
    ):
        result.append(
            {
                "path": f"schematic {verb}",
                "type": "query",
                "description": description,
                "output_schema": "pin_assignment",
                ...
                "params": [
                    {
                        ...
                        "multiple": False,
                        "description": (
                            "Saved snapshot object or successful 1.0 JSON envel"
                            "ope; project, revision, components required. No de"
                            "faults or native refresh."
                        ),
                    },
                    {
                        "name": "file",
                        ...
                        "input_contract": {
                            ...
                            "required_columns": ["refdes", "pin", "net"],
                            "optional_columns": ["expected_net"],
                            "additional_columns": False,
                            "unique_by": ["refdes", "pin"],
                            "identifiers": (
                                "exact strings; no integer conversion or whitespace normalization"
                            ),
                            "maximum_rows": MAX_ASSIGNMENTS,
                            "expected_net": (
                                "When column exists, blank means require explicit u"
                                "nconnected state for planning; ignored by pin-chec"
                                "k, which checks desired net."
                            ),
                        },
                    },
                    {
                ...
                "observation_contract": {
                    ...
                    "results": (
                        "valid=false is an assessment, not a transport erro"
                        "r; check matches=null when only unknowns prevent a"
                        " decision"
                    ),
                },
            }
