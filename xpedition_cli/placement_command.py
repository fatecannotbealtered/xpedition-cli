...
def validate_cli(argv: list[str], positionals: list[str]) -> None:
    ...
    while i < len(argv):
        ...
        if name not in allowed:
            raise CLIError("E_USAGE", "option is not supported by this placement command", {"option": name})
        if name in seen:
        ...
        if name not in BOOL_FLAGS:
            if not sep:
                ...
                raise CLIError("E_USAGE", "placement option requires a nonempty value", {"option": name})
        i += 1
...
def _preview(native: Any, params: dict[str, Any], request: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise CLIError("E_SERVER", "native placement preview is incomplete or inconsistent") from error
    return result
...
def dispatch_placement(positionals: list[str], options: dict[str, Any]) -> dict[str, Any]:
    ...
    if positionals[1] == "placement-plan":
        if options.get("backend", "mock") != "mock":
            raise CLIError("E_USAGE", "placement-plan is offline; use input observations, not a native backend")
        ...
        if not isinstance(observation, dict) or set(observation) != {"schema_version", "components"} or observation["schema_version"] != "1.0":
            raise CLIError("E_VALIDATION", "observation document needs schema_version and components")
        return plan_placement(request, observation["components"])
    if options.get("backend") != "native_xpedition":
        raise CLIError("E_BACKEND_UNAVAILABLE", "pcb placement requires explicit --backend native_xpedition")
    ...
    scope = {"operation": "pcb_placement", "backend": "native_xpedition", "project": str(path),
             "pcb": preview["pcb"], "request": request, "state_digest": preview["state_digest"]}
    if options.get("dry_run"):
        token, expiry = issue(scope)
        preview["risk"] = {
            "tier": "T1", "blast_radius": "selected placements and a board save; traces are not rerouted",
            "partial_failure": "previously moved parts may remain changed and the current part may be unplaced",
        }
        return {"preview": preview, "confirm_token": token, "expires_at": expiry, "_untrusted": ["preview"]}
    ...
    try:
        result = native.invoke("placement_batch", {**params, "apply": True,
                               "state_digest": preview["state_digest"]}, timeout_seconds=600.0)
    except CLIError as error:
        if error.code == "E_CONFLICT" and (error.details or {}).get("write_attempted") is False:
            raise
        raise CLIError("E_PROJECT_INVALID", "placement outcome is unknown; inspect the board before another write",
                       {"stage": "submit", "write_attempted": True, "outcome": "unknown",
                        "cause_code": error.code, "retry_safe": False}) from error
    if (not isinstance(result, dict) or result.get("outcome") != "complete"
            or not isinstance(result.get("verification"), dict)
            or result["verification"].get("valid") is not True):
        raise CLIError("E_PROJECT_INVALID", "placement did not complete; inspect per-item outcomes",
    ...
    if (not isinstance(results, list) or len(results) != len(request["selection"])
            or any(not isinstance(row, dict) or row.get("id") != name or row.get("ok") is not True
                   for row, name in zip(results, request["selection"], strict=True))):
        raise CLIError("E_PROJECT_INVALID", "placement response did not account for every selected component",
    ...
    for planned, returned in zip(preview["results"], results, strict=True):
        ...
        if not isinstance(observed, dict) or not same_position(planned["target"], observed):
            raise CLIError("E_PROJECT_INVALID", "placement response did not verify the requested target",
                           {"stage": "response", "write_attempted": True, "outcome": "unknown"})
    if preview["summary"]["changed_count"] and (
        result.get("write_attempted") is not True or result.get("drc_restored") is not True
    ):
        raise CLIError("E_PROJECT_INVALID", "placement execution or DRC restoration is unconfirmed",
                       {"stage": "response", "write_attempted": True})
    if result.get("write_attempted") is True and result.get("saved") is not True:
        raise CLIError("E_PROJECT_INVALID", "placement save is unconfirmed", {"stage": "save", "write_attempted": True})
    return result
