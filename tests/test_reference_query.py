...
@pytest.mark.parametrize("path", ["pcb trace", "pcb move", "schematic draw", "change apply", "context"])
def test_command_keeps_success_and_preview_schemas(path):
    ...
    for key in ("tool", "version", "release_readiness", "global_flags", "error_codes", "exit_codes"):
...
@pytest.mark.parametrize("args", [
    ("--command", "pcb trace"), ("--command=pcb trace",),
    ("--domain", "schematic"), ("--schema", "context"),
    ("--command", "  pcb   move  "),
])
def test_reference_selectors_through_cli(tmp_path, args):
    ...
    contract = json.loads((Path(__file__).resolve().parent.parent / "contract/contract.json").read_text())
...
@pytest.mark.parametrize("command", [
    ["project", "init"], ["pcb", "move"], ["agent", "serve"], ["version"],
])
def test_reference_flags_cannot_be_ignored_by_a_different_command(tmp_path, command):
    result = run_cli(*command, "--command", "context", "--dry-run",
                     "--project", str(tmp_path / "must-not-exist.json"),
