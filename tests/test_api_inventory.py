...
class TypeInfo:
    ...
    def GetTypeAttr(self):
        ...
        return NS(iid='{FAKE}', typekind=4, cFuncs=2, cVars=1, cImplTypes=0,
                  wTypeFlags=0, wMajorVerNum=1, wMinorVerNum=0)
    ...
    def GetFuncDesc(self, index):
        ...
        return NS(memid=7, args=((3, 1, None), (8, 48, 'do-not-print-default')),
                  invkind=2 if index == 0 else 4, wFuncFlags=0, callconv=4,
                  cParamsOpt=1, rettype=(24, 0, None))
...
@pytest.mark.parametrize('suffix,data', [('.exe', b'MZ\0\0'), ('.dll', b'MSFT'),
    ('.tlb', b'MZ\0\0'), ('.olb', b'bad')])
...
@pytest.mark.parametrize('args', [['--backend', 'native_xpedition'], ['--confirm', 'ct_secret'],
    ['--dry-run'], ['--input', '--name', 'A'], ['--input=x', '--input=y'], ['--output', 'x'],
    ['--json', '--format=json']])
...
def test_fields_cannot_hide_scope_execution_or_incompleteness():
    ...
    assert {'complete_in_scope','execution','source','_untrusted','has_more'} <= set(fields.split(','))
