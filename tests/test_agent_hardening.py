...
@pytest.mark.parametrize(
    "backend,expected", [("native_xpedition", "E_USAGE"), ("invalid", "E_VALIDATION")]
)
def test_project_init_does_not_fall_back_to_mock(tmp_path, monkeypatch, capsys, backend, expected):
    ...
    code, result = run(
        capsys, "project", "init", "--backend", backend, "--project", str(target), "--dry-run"
    )
    assert code == 2
...
def test_projection_through_cli_keeps_paging_and_trust(tmp_path, monkeypatch, capsys):
    ...
    path.write_text(
        json.dumps(
            {
                "project": "demo",
                "components": [{"refdes": "R1", "value": "10k"}, {"refdes": "C1", "value": "100n"}],
            }
        ),
        encoding="utf-8",
    )
...
def test_native_apply_reports_observed_postconditions(tmp_path, monkeypatch, capsys, mode):
    ...
    before = normalise_project(
        {"project": "demo", "components": [{"refdes": "R1", "x": 0, "y": 0}]}
    )
    ...
    changeset.write_text(
        json.dumps({"project": "demo", "operations": operations}), encoding="utf-8"
    )
    args = (
        "schematic",
        "apply",
    ...
    if mode in {"matched", "save_unreported"}:
        ...
        assert result["data"]["verification"]["saved"] is (
            None if mode == "save_unreported" else True
        )
    else:
        assert code == 2 and result["error"]["code"] == "E_PROJECT_INVALID"
        assert result["error"]["retryable"] is False
        assert result["error"]["details"]["write_attempted"] is True
        assert result["error"]["details"]["stage"] == (
            "read_back" if mode == "readback_timeout" else "verify"
        )
