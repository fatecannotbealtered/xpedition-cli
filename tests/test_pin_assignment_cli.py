...
def files(tmp_path):
    ...
    source.write_text(json.dumps({"project": "fixture", "revision": "R1", "components": [
        {"refdes": "J1", "pins": [{"number": "01", "net": "OLD"}, {"number": "1", "net": None}]}]}), encoding="utf-8")
...
def test_pin_commands_are_offline_queries(tmp_path, monkeypatch, capsys, verb):
    ...
    assert result["data"]["execution"] == {"supported": False, "performed": False,
...
@pytest.mark.parametrize("args", [["--backend", "native_xpedition"], ["--dry-run"],
    ["--confirm", "ct_do_not_use"], ["--project", "real.prj"], ["--input", "--file", "x"],
    ["--input=x", "--input=y"], ["--output", "real.prj"], ["extra"], ["--limit", "0"],
    ["--limit", "1001"]])
...
def test_field_selection_keeps_assessment_paging_and_trust(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("XPEDITION_CLI_CONFIG_DIR", str(tmp_path / "config"))
    source, pins = files(tmp_path)
    code, result = invoke(capsys, ["--compact", "schematic", "pin-plan", "--input", str(source),
        "--file", str(pins), "--limit", "1", "--fields", "items"])
    assert code == 0
    data = result["data"]
...
def test_unknown_observation_never_reports_matches_true(tmp_path, monkeypatch, capsys):
    ...
    source, pins = files(tmp_path)
    source.write_text(json.dumps({"project": "fixture", "revision": "R1", "components": [
        {"refdes": "J1", "pins": [{"number": "01"}, {"number": "1"}]}]}), encoding="utf-8")
    code, result = invoke(capsys, ["schematic", "pin-check", "--input", str(source), "--file", str(pins)])
