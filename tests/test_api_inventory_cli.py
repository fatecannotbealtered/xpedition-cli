...
@pytest.mark.parametrize('args', [['--project', 'x.prj'], ['--backend', 'native_xpedition'],
    ['--confirm', 'ct_secret'], ['--dry-run'], ['--output', 'x'], ['--input', '--name', 'X'],
    ['--name=X', '--name=Y'], ['extra']])
def test_inventory_rejects_unsafe_or_ambiguous_flags_before_loading(tmp_path, monkeypatch, capsys, args):
    monkeypatch.setenv('XPEDITION_CLI_CONFIG_DIR', str(tmp_path / 'config'))
...
def test_inventory_projection_retains_limitations(tmp_path, monkeypatch, capsys):
    ...
    source = fixture_file(tmp_path)
    monkeypatch.setattr(inventory, '_pythoncom', lambda: Com())
    code, result = invoke(capsys, 'system', 'api-inventory', '--input', str(source),
        '--name', 'IFixture', '--limit', '1', '--fields', 'items')
    assert code == 0
    value = result['data']
