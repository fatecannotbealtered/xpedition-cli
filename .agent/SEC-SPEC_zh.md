...
- **依赖锁定 + 审计**：提交 lockfile；对工具**分发所涉的每一个生态**都要在 CI 里跑依赖审计，
  ...
  release pipeline 在签名或发布任何东西之前，跑同样的审计和 spec/契约漂移守卫（仓库规范 §5c）：否则最后一次合并变绿之后
  才公布的漏洞，会随着一个签了名、却没人检查过的发布一起出去。
...
- [ ] 分发 checksum 校验，不匹配硬失败；release checksum 已签名或签名状态被显式报告；依赖锁定 + 审计，分发涉及的每个生态都审，合并路径和发布路径都跑
